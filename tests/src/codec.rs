...
pub fn encode_decode_test(blob: &[u8], test_body: &Vec<TestBody>) -> Result<(), ReadError> {
    ...
    fn run_refine_context_test() {
        let test_content = misc::read_bin_file(std::path::Path::new(&format!("{}/refine_context.bin", TEST_DIR))).unwrap();
    ...
    fn run_work_item_test() {
        let test_content = misc::read_bin_file(std::path::Path::new(&format!("{}/work_item.bin", TEST_DIR))).unwrap();
    ...
    fn run_work_package_test() {
        let test_content = misc::read_bin_file(std::path::Path::new(&format!("{}/work_package.bin", TEST_DIR))).unwrap();
    ...
    fn run_work_result_0() {
        let test_content = misc::read_bin_file(std::path::Path::new(&format!("{}/work_result_0.bin", TEST_DIR))).unwrap();
    ...
    fn run_work_result_1() {
        let test_content = misc::read_bin_file(std::path::Path::new(&format!("{}/work_result_1.bin", TEST_DIR))).unwrap();
        let test_body: Vec<TestBody> = vec![TestBody::WorkResult];
        let result = encode_decode_test(&test_content, &test_body);
        assert_eq!(result.is_ok(), true);
    }
    ...
    #[test]
    fn run_work_report() {
        let test_content = misc::read_bin_file(std::path::Path::new(&format!("{}/work_report.bin", TEST_DIR))).unwrap();
    ...
    fn run_tickets_extrinsic() {
        let test_content = misc::read_bin_file(std::path::Path::new(&format!("{}/tickets_extrinsic.bin", TEST_DIR))).unwrap();
   ...
   #[test]
    fn run_disputes_extrinsic() {
        let test_content = misc::read_bin_file(std::path::Path::new(&format!("{}/disputes_extrinsic.bin", TEST_DIR))).unwrap();
    ...
    fn run_preimages_extrinsic() {
        let test_content = misc::read_bin_file(std::path::Path::new(&format!("{}/preimages_extrinsic.bin", TEST_DIR))).unwrap();
        let mut reader = BytesReader::new(&test_content);
    ...
    fn run_assurances_extrinsic() {
        let test_content = misc::read_bin_file(std::path::Path::new(&format!("{}/assurances_extrinsic.bin", TEST_DIR))).unwrap();
        let mut reader = BytesReader::new(&test_content);
    ...
    fn run_guarantees_extrinsic() {
        let test_content = misc::read_bin_file(std::path::Path::new(&format!("{}/guarantees_extrinsic.bin", TEST_DIR))).unwrap();
        let mut reader = BytesReader::new(&test_content);
    ...
    #[test]
    fn run_extrinsic() {
        let test_content = misc::read_bin_file(std::path::Path::new(&format!("{}/extrinsic.bin", TEST_DIR))).unwrap();
    ...
    #[test]
    fn run_header_0() {
        let test_content = misc::read_bin_file(std::path::Path::new(&format!("{}/header_0.bin", TEST_DIR))).unwrap();
    ...
    #[test]
    fn run_header_1() {
        let test_content = misc::read_bin_file(std::path::Path::new(&format!("{}/header_1.bin", TEST_DIR))).unwrap();
        let test_body: Vec<TestBody> = vec![TestBody::Header];
        let result = encode_decode_test(&test_content, &test_body);
        assert_eq!(result.is_ok(), true);
    }
    ...
    #[test]
    fn run_block() {
        let test_content = misc::read_bin_file(std::path::Path::new(&format!("{}/block.bin", TEST_DIR))).unwrap();
