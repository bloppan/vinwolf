...
use utils::misc::hex_to_32;

#[derive(Clone)]
pub struct RawIdentity {
    pub seed: &'static str,
    pub ed25519_secret_seed: &'static str,
    pub ed25519_public: &'static str,
    pub bandersnatch_secret_seed: &'static str,
    pub bandersnatch_public: &'static str,
    pub dns_alt_name: &'static str,
}

#[derive(Clone, Debug)]
pub struct Identity {
    pub seed: OpaqueHash,
    pub ed25519_secret_seed: OpaqueHash,
    pub ed25519_public: Ed25519Public,
    pub bandersnatch_secret_seed: OpaqueHash,
    pub bandersnatch_public: BandersnatchPublic,
    pub dns_alt_name: String,
}

static DEV_ACCOUNTS: LazyLock<Mutex<HashMap<BandersnatchPublic, Connection>>> = LazyLock::new(|| Mutex::new(HashMap::new()));

pub fn add_dev_account(bandersnatch_public: BandersnatchPublic, connection: Connection) {
    let mut dev_accounts = DEV_ACCOUNTS.lock().unwrap();
    dev_accounts.insert(bandersnatch_public, connection);
}
...
pub fn parse_dev_accounts() -> [Identity; 6] {
    ...
    let raw = RAW_IDENTITIES.clone();
    [
        Identity {
            seed: hex_to_32(raw[0].seed),
            ed25519_secret_seed: hex_to_32(raw[0].ed25519_secret_seed),
            ed25519_public: hex_to_32(raw[0].ed25519_public),
            bandersnatch_secret_seed: hex_to_32(raw[0].bandersnatch_secret_seed),
            bandersnatch_public: hex_to_32(raw[0].bandersnatch_public),
            dns_alt_name: raw[0].dns_alt_name.to_string(),
        },
        Identity {
            seed: hex_to_32(raw[1].seed),
            ed25519_secret_seed: hex_to_32(raw[1].ed25519_secret_seed),
            ed25519_public: hex_to_32(raw[1].ed25519_public),
            bandersnatch_secret_seed: hex_to_32(raw[1].bandersnatch_secret_seed),
            bandersnatch_public: hex_to_32(raw[1].bandersnatch_public),
            dns_alt_name: raw[1].dns_alt_name.to_string(),
        },
        Identity {
            seed: hex_to_32(raw[2].seed),
            ed25519_secret_seed: hex_to_32(raw[2].ed25519_secret_seed),
            ed25519_public: hex_to_32(raw[2].ed25519_public),
            bandersnatch_secret_seed: hex_to_32(raw[2].bandersnatch_secret_seed),
            bandersnatch_public: hex_to_32(raw[2].bandersnatch_public),
            dns_alt_name: raw[2].dns_alt_name.to_string(),
        },
        Identity {
            seed: hex_to_32(raw[3].seed),
            ed25519_secret_seed: hex_to_32(raw[3].ed25519_secret_seed),
            ed25519_public: hex_to_32(raw[3].ed25519_public),
            bandersnatch_secret_seed: hex_to_32(raw[3].bandersnatch_secret_seed),
            bandersnatch_public: hex_to_32(raw[3].bandersnatch_public),
            dns_alt_name: raw[3].dns_alt_name.to_string(),
        },
        Identity {
            seed: hex_to_32(raw[4].seed),
            ed25519_secret_seed: hex_to_32(raw[4].ed25519_secret_seed),
            ed25519_public: hex_to_32(raw[4].ed25519_public),
            bandersnatch_secret_seed: hex_to_32(raw[4].bandersnatch_secret_seed),
            bandersnatch_public: hex_to_32(raw[4].bandersnatch_public),
            dns_alt_name: raw[4].dns_alt_name.to_string(),
        },
        Identity {
            seed: hex_to_32(raw[5].seed),
            ed25519_secret_seed: hex_to_32(raw[5].ed25519_secret_seed),
            ed25519_public: hex_to_32(raw[5].ed25519_public),
            bandersnatch_secret_seed: hex_to_32(raw[5].bandersnatch_secret_seed),
            bandersnatch_public: hex_to_32(raw[5].bandersnatch_public),
            dns_alt_name: raw[5].dns_alt_name.to_string(),
        },
    ]
}

static RAW_IDENTITIES: [RawIdentity; 6] = [
    RawIdentity {
...
mod tests {
    ...
    #[test]
    fn parse_dev_accounts_test() {
    
        let identities = parse_dev_accounts();
        println!("identities: {:x?}", identities);
        println!("parsed {} identities", identities.len());
    }
}